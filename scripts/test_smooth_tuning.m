% test_smooth_tuning.m
function test_smooth_tuning()
    rootDir = pwd;
    ...
    % Let's tune speed PI controller properly:
    % P = 0.003 Nm/rpm (gives 3 Nm per 1000 rpm error)
    % I = 0.03 Nm/(rpm*s)
    set_param('foc_speedcontroller/PI Controller', 'Controller', 'PI');
    set_param('foc_speedcontroller/PI Controller', 'P', '0.003');
    set_param('foc_speedcontroller/PI Controller', 'I', '0.03');
    ...
    fprintf('Running simulation with smooth speed PI controller...\n');
    ...
    title('PMSM FOC Drive: MTPA & Field Weakening Response (Smooth Tracking)', 'FontSize', 13, 'FontWeight', 'bold');
    ...
    saveas(f, 'smooth_multi_step_validation.png');
    close(f);
    
    fprintf('Saved smooth plot to smooth_multi_step_validation.png\n');
end
