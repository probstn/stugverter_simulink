...
set_param(harness, 'StopTime', '0.006', 'MaxStep', '1e-5');
out = sim(harness);
...
fprintf('Locked Rotor Step Response (iq_ref = 10 A at t=2 ms):\n');
for t_val = [0, 0.001, 0.002, 0.0025, 0.003, 0.004, 0.005, 0.006]
    id_v = interp1(id_act.Time, id_act.Data, t_val);
    iq_v = interp1(iq_act.Time, iq_act.Data, t_val);
    fprintf('t=%5.2f ms: id_act = %6.2f A, iq_act = %6.2f A\n', t_val*1000, id_v, iq_v);
end
