...
%% 1. Motor Parameters (IPMSM with Fischer Datasheet baseline & Ld != Lq)
pmsm.P        = 4;          % Pole pairs
...
pmsm.B        = 1e-4;       % Damping coefficient [Nm/(rad/s)]
...
% PM Flux linkage: lambda_pm = Ke_data / (sqrt(1.5)*P)
pmsm.fl       = 0.0604;     % [Wb]
...
Kt = single(1.5 * pmsm.P * lambda_pm); % Torque constant [Nm/A]

% Speed Loop Tuning (15 Hz bandwidth, critical damping zeta = 1.0)
...
fprintf('Setup configured cleanly! Ready for regime validation.\n');
