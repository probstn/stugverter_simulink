...
% Apply open-loop constant duty cycle corresponding to Valpha = 10 V, Vbeta = 0 V
% In PWM Reference Generator:
% Valpha_pu = 10 * sqrt(3) / 600 = 0.0288675
% Let's see what duty cycles are produced
harness = 'test_open_loop';
new_system(harness);
load_system(harness);
...
add_block('simulink/Sources/Constant', [harness '/Valpha_pu'], 'Value', '10*sqrt(3)/pmsm.V_rated');
add_block('simulink/Sources/Constant', [harness '/Vbeta_pu'], 'Value', '0');
...
fprintf('At t = 5 ms (Steady state for 10V Vd applied, R=0.126 -> expected id = 10/0.126 = 79.37 A):\n');
