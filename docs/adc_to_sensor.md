...
algorithm model referenced by the SIL variant in `stugverter_sim`, so conversion and
...
emulation remains in `stugverter_sim`; it is not part of the algorithm deployment.
...
- Existing 0.8 s `stugverter_sim` SIL acceptance checks passed; peak speed 18169.4 RPM,
